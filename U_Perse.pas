...
// Converte Serviços Prestados para Alíquota Zero CST 06----------------------------------------
...
// Lança os Registros M400 (PIS)  para o CST 06 de Serviços Prestados -------------------
...
// Lança os Registros M800 (COFINS) para o CST 06 de Serviços Prestados --------------------
...
// Converte Vendas A&B para Alíquota Zero CST 06----------------------------------------
...
// Lança os Registros M400 (PIS)  para o CST 06 de Vendas do A&B -------------------
  with FRM_Principal.SpedPC.Bloco_M.RegistroM400New do begin
   CST_PIS := stpisAliquotaZero;
...
// Lança os Registros M800 (COFINS) para o CST 06 de Vendas do A&B --------------------
  with FRM_Principal.SpedPC.Bloco_M.RegistroM800New do begin
   CST_COFINS := stcofinsAliquotaZero;
   VL_TOT_REC := Total_Nao_Cumulativo;
   COD_CTA := '3.01.01.02.001';
...
// Zeramento dos Créditos por entrada de mercadorias dos CST 50 e 60 -------------------
...
// Retirada do Crédito de Energia Elétrica CST 50 ----------------------------------
  with FRM_Principal.SpedPC.Bloco_C.RegistroC001 do begin
   for I := 0 to RegistroC010.Count - 1 do begin
    with RegistroC010.Items[I] do begin
...
// Retirada do Crédito de Depreciação e Amortização, Altera Retenção de Órgãos Públicos CST 50 ----------------------------------
  with FRM_Principal.SpedPC.Bloco_F do begin
   with RegistroF001.RegistroF010.Items[0] do begin
    ...
    RegistroF120.Clear;
//  RegistroF600.Clear;
    for i:= 0 to RegistroF600.Count -1 do begin
     with RegistroF600.Items[i] do
       IND_NAT_REC := inrNaoCumulativa;
    end;
   end;
  end;
  with FRM_Principal.SpedPC.Bloco_M do begin
   with RegistroM001 do begin
    RegistroM200.VL_RET_NC := RegistroM200.VL_RET_CUM;
    RegistroM600.VL_RET_NC := RegistroM600.VL_RET_CUM;
    RegistroM200.VL_RET_CUM := 0.00;
    RegistroM600.VL_RET_CUM := 0.00;
    RegistroM200.RegistroM205.Items[0].NUM_CAMPO := '08';
    RegistroM200.RegistroM205.Items[0].COD_REC := '691201';
    RegistroM600.RegistroM605.Items[0].NUM_CAMPO := '08';
    RegistroM600.RegistroM605.Items[0].COD_REC := '585601';
   end;
  end;
// Remove créditos sobre locações (maquinetas) CST 50 ---------------------------------
  with FRM_Principal.SpedPC.Bloco_F do begin
